...
class RecordVideo(
    ...
    """Records videos of environment episodes using the environment's render function and logs to WandB.
    ...
    def stop_recording(self):
        ...
        if len(self.recorded_frames) == 0:
            ...
            # wandb.log({f"video_{self._video_name}": wandb.Video(np.array(list(clip.iter_frames())), fps=self.frames_per_sec)})
            moviepy_logger = None if self.disable_logger else "bar"
            path = os.path.join(self.video_folder, f"{self._video_name}.mp4")
            clip.write_videofile(path, logger=moviepy_logger)
            wandb.log({f"video_{self._video_name}": wandb.Video(path)})
