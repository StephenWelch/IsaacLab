...
class track_ang_mom_z_world_exp(ManagerTermBase):
    ...
    def __call__(self, env: ManagerBasedRLEnv, command_name: str, std: float, gamma: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
        ang_vel_cmd = torch.zeros(env.num_envs, 3, device=env.device, dtype=torch.float32)
        ang_vel_cmd[:, 2] = env.command_manager.get_command(command_name)[:, 2]
        
        # Actual
        q = self.pinocchio_helper.get_q()
        v = self.pinocchio_helper.get_v()

        Ig, Ag, hg = self.ccrba([q, v])

        des_hg_z = gamma * Ig.view(-1, 3, 3)[:, 2, 2] * ang_vel_cmd[:, 2]

        return torch.exp(-torch.square(hg[:, 2] - des_hg_z) / std**2)
